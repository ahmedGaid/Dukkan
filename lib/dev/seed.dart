// Dev-only demo dataset writer — NOT part of the shipping app (nothing under
// lib/dev is imported from lib/main.dart). [runSeed] is the reusable core,
// called from two places:
//  1. `lib/dev/seed_demo_data.dart`'s thin `main()` — the CLI/device path
//     (`flutter run -t lib/dev/seed_demo_data.dart -d <device>`), unchanged
//     behavior from before this file existed.
//  2. The console devtools page (FC15, `system.tools`, founder-only, dev
//     project only) — «إعادة تعبئة البيانات التجريبية» calls this in place,
//     so re-seeding from a signed-in console session never needs the
//     separate entrypoint. Note this DOES sign the caller out at the end,
//     same as the CLI path — a console-triggered reseed always lands back
//     on the login screen, which is expected for this destructive tool.
//
// Writes: 1 owner (7 shops, ~53 products, 2 shop collections), 2 couriers
// (real Firebase Auth accounts), 3 customers (real accounts, profiles,
// favorites, and orders spanning every OrderStatus), plus the seed-managed
// taxonomy/areas/config/roles.
//
...
/// Runs the demo seed against [db]. Each phase is independently toggleable
/// so a partial re-seed (e.g. just RBAC after editing a role's permission
/// list) doesn't have to redo the whole ~15s pass. `catalog` always seeds
/// the 2 courier accounts first (driver docs are keyed by their real uids);
/// `customers` re-derives the owner/courier1 uids via idempotent
/// sign-in-or-create even when `catalog` is false in the same call, so a
/// customers-only re-seed still works once a catalog pass has run before.
Future<void> runSeed(
  ...
  if (catalog) {
    await _seedCatalog(db, ownerUid, courierUids, log);
  }
  if (rbac) {
    await _seedRbac(db);
    log.writeln('Wrote 4 roles and the founder admin.');
  }

  if (customers) {
    final courier1Uid = courierUids.isNotEmpty
        ? courierUids.first
        : await _signInOrCreate(
  ...
  // next launch of the real app (or the console session that triggered
  // this) opens as the last seeded customer instead of whoever was signed
  // in before. Sign out so the app returns to login.
  await auth.signOut();
}
...
Future<void> _seedCatalog(
  ...
  await _seedMigrationsDoc(firestore);
  ...
  log.writeln('Wrote ${_demoShops(ownerUid).length} shops, '
      ...
      'and ${courierUids.length} couriers.');
}
...
/// `/config/migrations` (FC15) — pre-seeds the applied-ids tracking doc so
/// devtools can `update` it (`hasPerm('settings.edit')`) instead of needing
/// a Worker route just to `create` it — `/config` docs are `create: false`
/// for every client, same relax-then-restore requirement as `platform`.
Future<void> _seedMigrationsDoc(FirebaseFirestore firestore) async {
  await _retryAuthSettle(
    () => firestore.collection('config').doc('migrations').set({'applied': <String>[]}),
  );
}
...
Future<void> _seedDrivers(
  ...
  for (var i = 0; i < _couriers.length && i < courierUids.length; i++) {
